...
```powershell
# 先诊断 GitHub 连通性与 push 凭据
py scripts/deploy_to_github.py --check

# 默认 medium：含优化后的 PNG，不含维护脚本
py scripts/deploy_to_github.py

# 最小包：无 PNG，用户侧 Agent 按需出图（~2 MB）
py scripts/deploy_to_github.py --profile slim

# 维护者全量：含 tests 与 deploy 脚本
py scripts/deploy_to_github.py --profile full
```

慢网/间歇断连时：使用持久工作目录 `--work-dir`，避免每次临时全量克隆；首次克隆成功后后续 deploy 仅 `git fetch`。

已有 PNG 批量转 medium：`python scripts/optimize_illustrations.py --pretty`
