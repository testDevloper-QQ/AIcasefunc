...
def save_png(data: bytes, path: Path, *, kind: str = "dish") -> Path:
    """Write PNG to disk with medium resize/optimize when kind is dish|step|ingredient."""
    from illustration_medium import IllustrationKind, write_medium_png

    k: IllustrationKind
    if kind in ("dish", "step", "ingredient"):
        k = kind  # type: ignore[assignment]
    else:
        k = "dish"
    write_medium_png(data, path, k)
    return path
